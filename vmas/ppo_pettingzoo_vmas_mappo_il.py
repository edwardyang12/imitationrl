...
def parse_args():
    ...
    # Optimized VMAS GPU Scaling configuration
...
if __name__ == "__main__":
    ...
    actions = torch.zeros((args.num_steps, actual_num_envs), device=device) 
    ...
    ep_returns = torch.zeros(actual_num_envs, device=device)
    ep_lengths = torch.zeros(actual_num_envs, device=device)
    ...
    for update in range(1, num_updates + 1):
        ...
        if update % 100 == 0:
            ...
            progress = (update - 1.0) / num_updates
            if progress < 0.7:
                # Evolve from 0.5 (Team) to 0.9 (Individualistic)
        ...
        for step in range(0, args.num_steps):
            ...
            if args.reward_cheat:
                ...
                if "terminal_raw_obs" in next_info:
                    eval_raw_obs = next_info["terminal_raw_obs"]
                else:
                    eval_raw_obs = next_info["raw_obs"]
                    
                new_landmark_dist = eval_raw_obs.view(num_games, num_agents_per_game, -1)[:, :, 4:4+2*args.num_landmarks]
                ...
                # FIX 1: Assign Targets FIRST if the previous step commanded a reset
                ...
                # Evaluate distance based on current assignments
            ...
            # FIX 1B: If the current step ended the game, FLAG it to re-assign on the NEXT step!
            game_dones = done.view(num_games, num_agents_per_game)[:, 0].bool()
            needs_assignment = needs_assignment | game_dones    
                
            next_done = done
            
            # FIX 2: Accumulate the true engine reward, NOT the cheat reward, for exact metric parity
            ep_returns += reward.view(-1)
            ep_lengths += 1
            ...
            if done.any():
                ...
                if done_games.any():
                    avg_return = ep_returns.view(num_games, num_agents_per_game)[done_games, 0].mean().item()
                    avg_length = ep_lengths.view(num_games, num_agents_per_game)[done_games, 0].mean().item()
                    ...
                    print(f"global_step={global_step}, avg_episodic_return={avg_return}")
                
                ep_returns[done_bool] = 0
                ep_lengths[done_bool] = 0

        with torch.no_grad():
            if "global_state" in next_info:
                if "terminal_raw_obs" in next_info:
                    eval_raw_obs = next_info["terminal_raw_obs"]
                    eval_global_state = next_info["terminal_global_state"]
                else:
                    eval_raw_obs = next_info["raw_obs"]
                    eval_global_state = next_info["global_state"]

                final_state = eval_global_state.view(num_games, -1)
                
                raw_obs_tensor = eval_raw_obs 
                landmark_dist = raw_obs_tensor.view(num_games, num_agents_per_game, -1)[:, :, 4:4+2*args.num_landmarks]
                landmark_dist = landmark_dist.view(num_games, num_agents_per_game, args.num_landmarks, 2)
                occupied = (torch.norm(landmark_dist, dim=-1) < strict_occupancy_radius).any(dim=1).float()
