...
def parse_args():
    ...
    parser.add_argument("--wandb-project-name", type=str, default="cleanRL_gnn_il",
...
class GraphObservationNormalizer(nn.Module):
    def __init__(self, n_max, feature_dim=8, continuous_dim=4, epsilon=1e-5):
        super().__init__()
        self.n_max = n_max
        self.total_nodes = 2 * n_max # ADD THIS
    ...
    def update(self, x_flat):
        B = x_flat.shape[0]
        # FIX: Reshape to total_nodes instead of n_max
        x_reshaped = x_flat.view(B, self.total_nodes, self.feature_dim)
    ...
    def normalize(self, x_flat):
        B = x_flat.shape[0]
        # FIX: Reshape to total_nodes instead of n_max
        x_reshaped = x_flat.view(B, self.total_nodes, self.feature_dim).clone()
...
class MultiHeadGATBackbone(nn.Module):
    def __init__(self, n_max, feature_dim=8, hidden_dim=64, out_dim=128, heads=4):
        super().__init__()
        self.n_max = n_max
        self.total_nodes = 2 * n_max  # 1 Ego + Max LMs + Max Other Agents
        self.feature_dim = feature_dim
    ...
    def _build_dynamic_graph(self, x_flat):
        ...
        # Use total_nodes for the tensor boundaries
        x_padded = x_flat.view(B, self.total_nodes, self.feature_dim)
        active_mask = x_padded[:, :, 6] > 0.5
        valid_x = x_padded[active_mask] 
        
        batch_indices = torch.arange(B, device=device).view(-1, 1).expand(B, self.total_nodes)
...
class GraphAgent(nn.Module):
    ...
    def get_value(self, x_flat, centralized_state, denormalize=False):
        ...
        total_nodes = 2 * self.n_max 
        density_scalar_per_agent = (active_counts.float() / total_nodes).view(-1, 1)
    ...
    def load_bc_weights(self, bc_model_path):
        ...
        print(f"\n[ORACLE] Successfully injected GNN BC weights from {bc_model_path}!")
...
class GraphObservationWrapper(BaseParallelWrapper):
    def __init__(self, env, n_max, current_n):
        super().__init__(env)
        self.n_max = n_max
        self.total_nodes = 2 * n_max # Internal graph capacity
        ...
        self.observation_spaces = {
            agent: gym.spaces.Box(
                ...
                shape=(self.total_nodes * self.feature_dim,), dtype=np.float32
    ...
    def process_obs(self, raw_obs, ego_idx):
        ...
        for i in range(N - 1):
            # Shift index by 1 (Ego) + n_max (Max Landmarks)
            nodes[1 + self.n_max + i] = [oa_rel_pos[i][0], oa_rel_pos[i][1], oa_vel[i][0], oa_vel[i][1], 0.0, 0.0, 1.0, other_tags[i]]
...
def make_env(args, env_id, seed, current_local_ratio=0.5):
    def thunk():
        ...
        env = GraphObservationWrapper(env, n_max=7, current_n=args.num_landmarks)
...
if __name__ == "__main__":
    ...
    n_max = 7 # Maximum number of agents/landmarks
    occupancy_dim = n_max
    ...
    if "global_state" in next_info:
        ...
        state_dim = (num_agents_per_game * np.array(envs.single_observation_space.shape).prod()) + occupancy_dim
    ...
    agent.load_bc_weights(f"gnn_student_bc_best_nmax{n_max}.pt")
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            if "global_state" in next_info:
                ...
                # 1. Reshape to the FIXED global graph structure
                obs_reshaped = next_obs.view(num_games, num_agents_per_game, 2 * n_max, 8)

                # 2. Extract ALL landmark nodes up to the global max capacity
                landmark_nodes = obs_reshaped[:, :, 1:1+occupancy_dim, :]

                # 3. Separate coordinates and active flags
                landmark_dist = landmark_nodes[:, :, :, 0:2]
                is_active = landmark_nodes[:, :, :, 6]

                # 4. Calculate raw distance
                close_enough = torch.norm(landmark_dist, dim=-1) < strict_occupancy_radius

                # 5. MASK: A landmark is only occupied if it is actually active
                valid_occupied = close_enough & (is_active > 0.5)

                # 6. Collapse across agents
                occupied = valid_occupied.any(dim=1).float() 
        ...
        with torch.no_grad():
            ...
            if "global_state" in next_info:
                final_game_states = bootstrap_obs.view(num_games, -1)

                # 1. Reshape to the FIXED global graph structure
                obs_reshaped = bootstrap_obs.view(num_games, num_agents_per_game, 2 * n_max, 8)
